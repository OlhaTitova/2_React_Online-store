...
class Card extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = {
            cardList: props.valueCard.vendor
        }
    }

    addToCart = () => {
        const cart = new Cart();
        cart.addItem(this.props.valueCard.vendor, 1);
    }
    ...
    addToFavorite = () => {
        ...
        favorite.addFavoriteItem(this.props.valueCard.vendor, true);

    }

    render() {
        const { name, url, price, vendor, color } = this.props.valueCard;
        return (
            <div className="Card" id={vendor}>
                <div className="row justify-cont-between">
                    <img className="Card__Img" src={url} alt={name} />
                    <button className="Card__Btn" onClick={() => this.addToFavorite}>
                        <svg id={vendor} width="20" height="17" fill="none" viewBox="0 0 20 17" xmlns="http://www.w3.org/2000/svg">
                ...
                <p className="Card__Vendor">Код: {vendor}</p>
                <p><span className="Card__Color">Цвет:</span>{color}</p>
                <div className="row justify-cont-between align-items-center mt-15">
                    <span className="Card__Price">{price} <span className="Card__Сurrency">грн</span></span>

                    {fields.map(({ ...item }, index) => {
                        return (
                            <Modal key={index} fields={item} onClick={() => this.addToCart} />
                        )
                    })}
                </div>
            </div>
